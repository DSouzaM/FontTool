...
class Environment(object):
    ...
    def binary_operation(self, action):
        self.program_stack_changes.extend(["read","read","pop","pop","push"])
	...
	self.current_instruction_intermediate.append(IR.ALIGHPTSMethodCall(args))
	...
	for k in range(0,loopValue):
        ...
        self.current_instruction_intermediate.append(IR.ALIGNRPMethodCall(pts,loop_value = loopValue))
	...
	self.program_stack_changes.extend(["read","read","read","read","read","pop","pop","pop","pop","pop"])
		...
		if dir == 1:
			...
			executor.environment = copy.deepcopy(if_stmt.environment)
                        if not if_stmt.current_func_tree is None:
                            executor.current_func_tree = copy.deepcopy(if_stmt.current_func_tree)
			    ...
			    #itr.vest = s.successors[-1]
					...
					executor.environment = copy.deepcopy(executor.if_else_stack[-num_of_layers_crossed].env_on_exit)
				  	if not executor.if_else_stack[-num_of_layers_crossed].current_func_tree is None:
                                            executor.current_func_tree = copy.deepcopy(executor.if_else_stack[-num_of_layers_crossed].current_func_tree) 
	...
	self.current_isntruction_intermediate.append(IR.CopyStatement(IR.Variable(var),dataType.PointSize))
	...
	self.program_stack_changes.extend(["read","read","pop","pop"])
        args = self.program_stack_pop_many(2)
        self.current_instruction_intermediate.append(IR.SFVTLMethodCall(self.current_instruction.data[0], args))
	...
	zp0 = IR.ZP0()
	zp0.identifier += ".all"
        self.current_instruction_intermediate.append(IR.CopyStatement(zp0, arg))
        self.current_instruction_intermediate.append(IR.CopyStatement(IR.ZP1(), arg))
        self.current_instruction_intermediate.append(IR.CopyStatement(IR.ZP2(), arg))
    ...
    def exec_WS(self):
        res = self.getOpandVar()
        self.current_instruction_intermediate.append(IR.WriteStorageStatement(res[3],res[2]))
...
class Executor(object):
	...
	erase_lines(self.row_to_erase)
        sys.stdout.write('-'*(font_name_len+time_len+memory_len+percentage_len+tag_len+6)+'\n')
        sys.stdout.write('|')
	...
	if self.current_tag == 'prep':
	    print '**************** working on prep ...'
  	    ...
  	    print '**************** working on glyph: ',str(self.glyph_num),'/',str(self.all_glyphs)
        self.row_to_erase = 6
	...
	if isinstance(callee,dataType.AbstractValue):
		...
		callee = self.uncertain_callee_backup.function_itr
		    ...
		    if len(self.uncertain_callee_backup.possible_functions)>0:
		 	...
		 	caller = self.call_stack[-1][0]
			self.replace_function_table[self.uncertain_callee_backup.possible_functions[0]] = self.uncertain_callee_backup.possible_functions
	...
	# stack_used (previous)
        for i in range(num_of_parameters):
            if i > 0:
                call_args += ', '
            call_arg_name = identifierGenerator.generateIdentifier(tag, stack_depth_upon_call - i )
	...
	if not self.current_tag == 'prep':
		...
		continue
			...
			if len(self.if_else_stack[-1].if_stmt.successors) == 3:
			    ...
			    if len(self.call_stack) > 0:
				len_of_stack_before_func = len(self.call_stack[-1][8])
			    difference1 = self.environment.program_stack_changes[len_of_stack_before_func:]
			    ...
			    if args2 > args1:
				self.environment.program_stack_changes = self.if_else_stack[-1].program_stack_changes_on_else
		    ...
		    self.environment.program_stack_changes.extend(["read","pop"])
			 ...
			 if store_if_instruction:
		             self.current_instruction.stored_environments = copy.deepcopy(self.stored_environments)
		             self.current_instruction.already_seen_insts = copy.deepcopy(self.already_seen_insts)
		             self.current_instruction.ignored_insts = copy.deepcopy(self.ignored_insts)
		             self.current_instruction.bytecode2ir = copy.deepcopy(self.bytecode2ir)
		             self.current_instruction.environment = copy.deepcopy(self.environment)
		             self.current_instruction.environment.program_stack.pop()
		             self.current_instruction.environment.program_stack.pop()
                             if not self.current_func_tree is None:
			         self.current_instruction.current_func_tree = copy.deepcopy(self.current_func_tree)
		             self.flag = 0 
	    ...
	    if store_env:
		    ...
		    if self.current_instruction.mnemonic != 'EIF':
			    ...
			    print 'this function does not work!!', len(self.environment.program_stack),len(self.stored_environments[self.current_instruction.id].program_stack)
			    sys.exit()
			else:
	    ...
	    # normal case: 1 succ
			...
			self.current_instruction = self.current_instruction.successors[0]
			  ...
			  for inst in block.else_instructions:
			      ...
			      self.if_else_stack[-1].program_stack_changes_on_else = copy.deepcopy(self.environment.program_stack_changes)
